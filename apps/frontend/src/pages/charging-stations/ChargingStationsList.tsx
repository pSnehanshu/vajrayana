...
export function ChargingStationsList() {
  ...
  const charginStationsListQuery = trpc.stations.list.useQuery({
    page: pageNum,
    take: itemsPerPage,
    search,
  });

  const itemsReceived = charginStationsListQuery.data?.stations.size ?? 0;
  const startSN = itemsReceived > 0 ? (pageNum - 1) * itemsPerPage + 1 : 0;
  const total = charginStationsListQuery.data?.total ?? 0;
      ...
      <div className="bg-white dark:bg-gray-800 sm:rounded-lg">
        <div className="flex flex-col-reverse lg:flex-row items-end lg:items-center justify-between relative w-full mb-2">
          <div className="w-full max-w-xl flex space-x-2">
            ...
            {charginStationsListQuery.isLoading && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {charginStationsListQuery.data?.stations &&
                Array.from(charginStationsListQuery.data?.stations).map(
                  ([, cs]) => {
                    return (
                      <tr
                        key={cs.id}
                      ...
                      >
                        ...
                        <td>
                          {cs.vendorName} {cs.model}{" "}
                          {cs.serialNumber ? `(SN: ${cs.serialNumber})` : ""}
                        </td>

                        <td className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                          {format(cs.createdAt, "do LLL yyy, hh:mm aaa")}
                        </td>

                        <td>
                          <Link
                            to={`/cs/stations/${cs.id}`}
                            className="block border m-1 py-2 px-1 text-center hover:bg-gray-800 rounded-lg border-gray-400"
                          >
                            View details
                          </Link>
                        </td>
